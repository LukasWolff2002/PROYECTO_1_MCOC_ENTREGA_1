...
def laplace (caso, nombre, grilla, factor_correccion, factor):
    ...
    nx, ny = grilla*2, grilla  # Tamaño de la grilla
    lx, ly = C1*2, C1  # Dimensiones físicas del dominio
    dx, dy = ly / (ny - 1), ly / (ny - 1)  # Espaciado en x e y
    #print(dx,dy)
    ...
    if C2 != ly:
        ...
        potential[c2-1, (nx)//4:(3*nx)//4] = Potencial_final   # Borde superior: potencial alto
        potential[0, (3*nx)//4:] = Potencial_incial     # Borde superior: potencial alto
        potential[0, 0:(nx)//4] = Potencial_incial
        ...
        potential[:D, nx//4] = 0    #Ataguia
        K[:D, nx//4] = 1e-15     #Ataguia
        potential[:D, (3*nx)//4-1] = 0    #Ataguia
        K[:D, (3*nx)//4-1] = 1e-15     #Ataguia
        potential[:c2-1, (nx)//4:(3*nx)//4] = np.nan     # Borde superior: potencial alto
        K[:c2-1, (nx)//4:(3*nx)//4] = 1e-15      # Borde superior: potencial alto

    else:
        
        potential[0, (nx)//4:(3*nx)//4] = Potencial_final   # Borde superior: potencial alto
        potential[0, (3*nx)//4:] = Potencial_incial     # Borde superior: potencial alto
        potential[0, 0:(nx)//4] = Potencial_incial
        ...
        for i in range(ny):
            for j in range(nx):
                ...
                # Verificar si estamos en la línea vertical de nx//2 y hasta ny//2
                    ...
                    if factor_correccion == True:

                        if i < ny-2 and potential[i+1, j] > 2 and j < factor:
                            #print(f'se cumple la condicion con {i=}')
                            suma_potenciales += potential[i+1, j]
                            vecinos_validos += 1

                        if i < ny-1 and potential[i+1, j] > 1 and j >= factor:
                            #print(f'se cumple la condicion con {i=}')
                            suma_potenciales += potential[i+1, j]
                            vecinos_validos += 1

                    elif factor_correccion == False:
    ...
    for i in range(ny):
        salida_velocity_y = velocity_y[i, :]  # Velocidad en y a lo largo del borde inferior
        a = 0
        for element in salida_velocity_y:
            if np.isnan(element):
                #print('hay un nan')
                a += 1
                break
    ...
    print('')
    salida_velocity_y = salida_velocity_y[(nx)//4:(3*nx)//4]
    print(f'Flujo de salida en y es{salida_velocity_y}')
    ...
    #print('')
    q = ((np.sum(salida_velocity_y)*espaciado*-1)/(C1*100/4))
    ...
    # Subplot para el potencial
    plt.subplot(1, 3, 2)
    plt.contourf(X_inverted, Y_inverted, potential/100, levels=18, cmap='Blues')
    ...
    masked_potential = np.ma.masked_where(potential == 0, potential)
    ...
    plt.tight_layout()
    plt.savefig(f"laplace_{nombre}_doble.jpg", format='jpg', bbox_inches='tight', pad_inches=0)

    print(f'listo {nombre}')
    print('')

#laplace(caso_1, 'caso_1', 60) #14 levels y 10 en el factor de correccion
#laplace(caso_2, 'caso_2', 40, False, 0) #14 levels y 10 en el factor de correccion
#laplace(caso_3, 'caso_3', 20, False, 30)
laplace(caso_ejemplo, 'caso_ejemplo', 60, False, 0) #No debo poner un j, convergencia alcanzada